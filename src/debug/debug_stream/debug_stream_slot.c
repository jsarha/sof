...
static const int slot_num = CONFIG_SOF_DEBUG_STREAM_SLOT_NUMBER;

static struct debug_stream_slot_hdr *get_my_slot(void)
{
	return (struct debug_stream_slot_hdr *)ADSP_DW->slots[slot_num];
}

static
struct debug_stream_circular_buf *get_circular_buffer(struct debug_stream_section_descriptor *desc,
						      unsigned int core)
{
	struct debug_stream_slot_hdr *hdr = get_my_slot();
	void *ptr;
	...
	*desc = hdr->section_desc[core];
	LOG_INF("Section %u (desc %u %u %u)", core, desc->core_id, desc->buf_words, desc->offset);
...
int debug_stream_slot_send_record(struct debug_stream_record *rec)
{
	...
	struct debug_stream_circular_buf *buf = get_circular_buffer(&desc, arch_proc_id());
	...
	if (rec->size >= desc.buf_words) {
		LOG_ERR("Record too big %u >= %u (desc %u %u %u)", rec->size,
			desc.buf_words, desc.core_id, desc.buf_words, desc.offset);
		return -ENOMEM;
	}
	k_mutex_lock(&cpu_mutex[arch_proc_id()].m, K_FOREVER);

	rec->serial = buf->next_serial++;
	rec->size = record_size + 1; /* +1 for size at the end of recored */
	...
	if (buf_remain < record_size) {
		...
		assert(!memcpy_s(&buf->data[record_start], buf_remain * sizeof(uint32_t),
				 rec, buf_remain * sizeof(uint32_t)));
		assert(!memcpy_s(&buf->data[0], desc.buf_words * sizeof(uint32_t),
...
static int debug_stream_slot_init(void)
{
	...
	if (ADSP_DW->descs[slot_num].type != 0)
		LOG_WRN("Slot %d was not free: %u", slot_num,
			ADSP_DW->descs[slot_num].type);
	ADSP_DW->descs[slot_num].type = ADSP_DW_SLOT_DEBUG_STREAM;
