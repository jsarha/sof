 ...
 *
 *      --------------------------------------------------  ---
 *      | next_serial = <counter for written objects>    |   |
 ...
 * circular buffer, and updates the w_ptr before it writes record.
 * The host side receiver tries to keep up with the w_ptr and keeps
 * track of its read position. The size of the record is written -
 * again - after each record and before the next. This is to allow
 * parsing the stream backwards in an overrun recovery situation. The
 * w_ptr value is updated last, when the record is completely written.
 */
