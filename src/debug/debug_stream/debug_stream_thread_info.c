...
static int thread_info_buf_realloc(struct record_buf *rec, size_t req_size)
{
	size_t size = rec->size;

	while (size < rec->w_ptr + req_size)
		size *= 2;
	...
	if (size != rec->size) {
		...
		memcpy_s(buf, size, rec->buf, rec->w_ptr);
		rfree(rec->buf);
		rec->size = size;
		rec->buf = buf;
	}
...
static void thread_info_cb(const struct k_thread *cthread, void *user_data)
{
	...
	if (!name || name[0] == '\0') {
		...
		if (thread_info_buf_realloc(ud->rec, sizeof(*tinfo) + ptr_str_len))
			return;

		tinfo = (struct thread_info *) &ud->rec->buf[ud->rec->w_ptr];
		snprintk(tinfo->name, ptr_str_len, "%p", (void *)thread);
		/* Drop the terminationg '\0', that is why -1 is here. */
		tinfo->name_len = ptr_str_len - 1;
		ud->rec->w_ptr += sizeof(*tinfo) + ptr_str_len - 1;
	} else {
		size_t len = strlen(name);

		if (thread_info_buf_realloc(ud->rec, sizeof(*tinfo) + len + 1))
			return;

		tinfo = (struct thread_info *) &ud->rec->buf[ud->rec->w_ptr];
		strncpy(tinfo->name, name, len + 1);
		tinfo->name_len = len;
		ud->rec->w_ptr += sizeof(*tinfo) + len;
	}
...
static void thread_info_get(int core, struct record_buf *rec)
{
	...
	hdr = (struct thread_info_record_hdr *) ud.rec->buf;
	rec->w_ptr = sizeof(*hdr);
	...
	hdr->hdr.size = SOF_DIV_ROUND_UP(rec->w_ptr, sizeof(hdr->hdr.data[0]));
...
static void thread_info_run(void *cnum, void *a, void *b)
{
	...
	rec.buf = rmalloc(SOF_MEM_ZONE_RUNTIME, 0, SOF_MEM_CAPS_RAM, rec.size);
	if (!rec.buf) {
