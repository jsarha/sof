...
class RecordPrinter:
    ...
    def print_record(self, record, cpu):
        ...
        logging.debug(
            "rec: %u %u %u", recp.contents.id, recp.contents.serial, recp.contents.size
        )
    ...
    def print_thread_info(self, record, cpu):
        ...
        print(
            "CPU %u: Load: %02.1f%% %u threads (serial %u)"
            % (
                cpu,
                cpup.contents.load / 2.55,
                cpup.contents.thread_count,
                cpup.contents.hdr.serial,
            )
        )
...
class DebugStreamSlotHdr(ctypes.Structure):
    ...
    _fields_ = [
        ...
        # This is for cacheline alignment
        ("padding", ctypes.c_ubyte * (64 - 2 * WSIZE - ctypes.sizeof(DebugStreamHdr))),
    ]
...
class CircularBufferDecoder:
    ...
    def get_hdr(self, slot, pos):
        ...
        if header.id > 100 or header.size >= self.buf_words:
            logging.warning(
                "Broken record id %u serial %u size %u",
                header.id,
                header.serial,
                header.size,
            )
    ...
    def get_record(self, slot, pos, serial):
        ...
        if rec is None or rec.size == 0:
            return None
        logging.debug(
            "got header at pos %u rec %u %u %u", pos, rec.id, rec.serial, rec.size
        )
        if serial is not None and rec.serial != serial:
            logging.warning(
                "Record serial mismatch %u != %u, pos %u size %u",
                rec.serial,
                serial,
                pos,
                rec.size,
    ...
    def catch_up(self, slot):
        ...
        logging.info("serial %u w_ptr %u", self.prev_serial, self.prev_w_ptr)

    def decode_past_records(self, slot, pos, serial):
        """
        Decode records backwards from pos. Should not be called directly, use catch_up()
        """
        if self.prev_serial is not None and self.prev_serial >= serial - 1:
            return
        if pos == 0:
        ...
        if record is not None:
            if not self.printer.print_record(record, self.cpu):
                logging.info("Parse failed on record %u", serial - 1)
            logging.info("Printing %u success", serial - 1)
        else:
            logging.info("Broken record %u", serial - 1)

    def get_next_record(self, slot):
        """
        Get next record from the circular buffer and print it, returns True if a record
        with expected serial number was found and successfully decoded and printed.
        """
        ...
        if record is not None:
            ...
            if print_success:
                recp = ctypes.cast(record, ctypes.POINTER(DebugStreamRecord))
                self.prev_w_ptr = (
                    self.prev_w_ptr + recp.contents.size
                ) % self.buf_words
                self.prev_serial = recp.contents.serial
            else:
                logging.info("Parse failed on record %u", self.prev_serial + 1)
...
class DebugStreamDecoder:
    ...
    def catch_up_all(self):
        ...
        is valid serial number available the decoding stops when there previous
    ...
    def poll(self):
        """
        Poll all circular buffers for more record. Returns True if nothing new
